...
CON = sqlite3.connect('////content/example.db')
...
def load_data(tmp_file, table_name, conn, **context):
    """ Load to DB
    """
    data = pd.read_csv(tmp_file)# Изменение read_csv
...
with DAG(dag_id='simple_etl',
         ...
         tags=["stepic"] # тэги для фильтрации дагов  
    ) as dag:
    ...
    load_data = PythonOperator(
        ...
        op_kwargs={
            ...
            'table_name': 'test_dag_aggregation',
            'conn': CON
        }
    )
    ...
    extract_data.doc_md = "Task Documentations :)"
    transform_data.doc_md = "Task Documentations :)"
    load_data.doc_md = "Task Documentations :)"
    email_op.doc_md = "Task Documentations :)"
